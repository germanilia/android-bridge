...
- [x] Increment 6: Trusted Presence (Mac) — a fifth tab where the user nominates trusted Wi-Fi networks (identified by router MAC, because macOS 26 redacts SSID/BSSID from unprivileged callers) and trusted paired Bluetooth devices (counted only while connected). Two independent switches, both off by default: hold the Mac awake so it never reaches the lock screen, and turn off the wake password via `sysadminctl -screenLock`. The login password lives in the login Keychain and is passed on stdin. Restores locking on sleep (default), on quit, and whenever the app — not the user — was the one that disabled it. Requires `NSBluetoothAlwaysUsageDescription` in Info.plist — without it macOS terminates the bundled app on the paired-device read, with no crash report. Design: `aidlc-docs/construction/trusted-presence/design.md`.
